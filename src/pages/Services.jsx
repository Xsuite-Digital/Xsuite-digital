...
useEffect(() =>{
  ...
  return (
    <section className="text-gray-600 body-font lg:mt-12">
          ...
          <div className="grid grid-cols-1 gap-y-12 text-center lg:grid-cols-3 gap-4 items-center justify-center ">
            <AnimatePresence>
              {displayedServices.map((data, index) => (
                ...
                >
                  <img src={data.imgSrc}  className='icon-container text-black  h-10 w-10'/>
                    
                  
                  <p className='text-xl font-bold text-black tracking-wide' >{data.title}</p>
      ...
      {isOpen && <ServicesModal openModal={openModal} service={selectedService} onClose={closeModal} />}
